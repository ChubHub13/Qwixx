(() => {
  ...
  </style><header class="top"><div class="mark">⚄</div><div class="title">QWIXX<small>Three players · shared-dice table</small></div><i class="spacer"></i><button class="settings" id="settingsButton">Settings</button></header><div class="shell"><aside class="side"><section class="panel round"><b id="round">Waiting</b><span id="roundHint">Choose a player to begin</span></section><section class="panel"><h2 class="score-title">SCORE · HIGH WINS</h2><div id="people"></div></section><div class="side-actions"><button id="start">Start Game</button></div></aside><main class="table"><div class="seats" id="seats"></div><div class="boards" id="boards"></div><section class="roll" id="roll"></section></main></div><section class="pop hide" id="settings"><h2>Table settings</h2><label class="option"><input name="count" type="radio" value="2"> 2 white dice</label><label class="option"><input name="count" type="radio" value="3"> 3 white dice</label><label class="option"><input name="count" type="radio" value="all3" checked> 3 white dice · can use all 3</label></section><section class="modal" id="modal"><div class="modal-card"><h1>Choose Your Player</h1><div class="players" id="players"></div></div></section><button class="return" id="return">← Game Night</button><div class="toast" id="toast"></div>`;
  document.body.insertAdjacentHTML('beforeend', `<style>
    ...
    .seats{pointer-events:none}.seat{pointer-events:auto;height:auto!important;min-height:0!important}.seat .avatar{width:39px;height:39px;border-radius:50%;object-fit:cover;border:2px solid #deb44e;background:#deb44e}.seat .avatar.guest-avatar{display:grid;place-items:center;color:#22180a;font-style:normal;font-size:23px;font-weight:900}.seat.under-left{top:310px!important;bottom:auto!important;left:18px!important;right:auto!important;transform:none!important}.seat.under-right{top:310px!important;bottom:auto!important;right:18px!important;left:auto!important;transform:none!important}.seat.beside-bottom{top:auto!important;bottom:55px!important;right:calc(50% - 32vw)!important;left:auto!important;transform:none!important}.row{grid-template-columns:repeat(11,minmax(9px,1fr)) 25px 36px!important;gap:2px!important}.row>i{display:none}.row-score,.row-lock{display:grid;place-items:center;border-radius:7px;background:#151619;color:#fff;font-size:14px;font-weight:900}.row-lock{background:#f7f4e7;border:1px solid #999;color:#111;font-size:14px}.row-lock img{width:15px;height:15px;object-fit:contain}.cell{font-size:18px!important;font-weight:900!important;opacity:1!important;border-radius:7px!important;color:#fff!important;text-shadow:none!important}.cell.mark{width:100%!important;height:28px!important;min-width:0!important;box-sizing:border-box!important;background:#aeaeae!important;text-shadow:none!important;font-size:18px!important}.row.red .cell.mark,.row.yellow .cell.mark,.row.green .cell.mark,.row.blue .cell.mark{background:#aeaeae!important}.row.red .cell.mark{color:var(--red)!important}.row.yellow .cell.mark{color:var(--yellow)!important}.row.green .cell.mark{color:var(--green)!important}.row.blue .cell.mark{color:var(--blue)!important}
    .cell,.cell.mark{text-shadow:none!important}.cell.mark{height:28px!important;min-width:0!important}.board-foot{min-height:38px}.side-actions{grid-template-columns:1fr}.seat-done{margin-left:3px;border:1px solid #a17b43;border-radius:6px;background:#391943;color:#fff1b9;padding:4px 7px;font-size:10px;font-weight:900}.done{display:block;margin:18px auto 0;border:1px solid #9c7b42;border-radius:8px;background:#311a37;color:#fff4c7;padding:9px 28px;font-weight:900}.players{grid-template-columns:repeat(2,1fr)}.players button.guest-choice{display:grid;place-items:center;gap:4px}.guest-choice-icon{font-size:25px;line-height:1}
  </style>`);
  document.head.insertAdjacentHTML('beforeend', '<style>#return{z-index:10000!important;pointer-events:auto!important}.players{grid-template-columns:repeat(3,1fr)}.players .join-seat{position:relative;display:block;min-width:0}.players .join-seat [data-player]{width:100%;min-width:0;padding-right:32px;text-align:center}.players .join-name-edit{position:absolute;top:50%;right:6px;display:grid;place-items:center;width:23px;height:23px;margin:0;padding:0;transform:translateY(-50%);border:0;border-radius:6px;background:transparent;font-size:16px;line-height:1}.players .join-name-edit:hover{background:rgba(255,240,180,.16)}.seat-name{display:flex;align-items:center;gap:3px}.seat .name-edit{display:grid;place-items:center;width:16px;height:16px;padding:0;border:0;border-radius:50%;background:#3a1949;color:#fff0b4;font:900 11px/1 system-ui,sans-serif;cursor:pointer}.seat .name-edit:hover{background:#754261}</style>');
  ...
  function render() {
    ...
    const rollingName=state.seats?.[state.rollingSeat ?? state.turn]?.name || names[state.rollingSeat ?? state.turn];
    ...
    $('#seats').innerHTML=state.seats.map(s=>{const isCustom=s.name!==names[s.seat];return `<div class="seat s${s.seat}">${isCustom?'<i class="avatar guest-avatar" aria-label="Guest">👤</i>':`<img class="avatar" src="${avatars[s.name]}" alt="${s.name}">`}<div class="seat-name"><b>${s.name}</b>${s.seat===state.you?`<button class="name-edit" type="button" data-edit-name="${s.seat}" aria-label="Edit ${s.name}'s name">✎</button>`:''}</div></div>`}).join(''); document.querySelectorAll('[data-edit-name]').forEach(button=>button.onclick=()=>editPlayerName(state.seats[Number(button.dataset.editName)]?.name||''));
  ...
  function editPlayerName(currentName){const name=window.prompt('Name for this game:',currentName);if(name===null)return;if(!name.trim()){say('Enter a name for this game.');return;}act({action:'rename',name});}
  ...
  async function join(name,displayName){try{const x=await api('/api/join',{name,...(displayName?{displayName}:{})});token=x.token;localStorage.setItem('juddQwixxToken',token);state=x.state;$('#modal').classList.add('hide');render()}catch(e){say(e.message)}}
  function promptJoinName(player){const name=window.prompt(`Name for ${player}'s seat:`,'');if(name===null)return;if(!name.trim()){say('Enter a name for this game.');return;}join(player,name);}
  $('#players').innerHTML=names.map(n=>`<div class="join-seat"><button data-player="${n}">${n}</button><button class="join-name-edit" data-edit-player="${n}" type="button" aria-label="Use ${n}'s seat with another name">✎</button></div>`).join(''); document.querySelectorAll('#players [data-player]').forEach(button=>button.onclick=()=>join(button.dataset.player)); document.querySelectorAll('#players [data-edit-player]').forEach(button=>button.onclick=()=>promptJoinName(button.dataset.editPlayer));
