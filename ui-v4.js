(() => {
  ...
  </style><header class="top"><div class="mark">⚄</div><div class="title">QWIXX<small>Three players · shared-dice table</small></div><i class="spacer"></i><button class="settings" id="settingsButton">Settings</button></header><div class="shell"><aside class="side"><section class="panel round"><b id="round">Waiting</b><span id="roundHint">Choose a player to begin</span></section><section class="panel"><h2 class="score-title">SCORE · HIGH WINS</h2><div id="people"></div></section><div class="side-actions"><button id="start">Start Game</button></div><section class="panel all-time"><div class="all-time-tabs"><button data-score-tab="high">High Scores</button><button data-score-tab="low">Low Scores</button></div><button class="all-time-filter" id="botScoreToggle" type="button">Hide Bots</button><ol id="allTimeScores" class="all-time-list"></ol></section></aside><main class="table"><div class="seats" id="seats"></div><div class="boards" id="boards"></div><section class="roll" id="roll"></section></main></div><section class="pop hide" id="settings"><h2>Table settings</h2><label class="option"><input name="count" type="radio" value="2"> 2 white dice</label><label class="option"><input name="count" type="radio" value="3"> 3 white dice</label><label class="option"><input name="count" type="radio" value="all3" checked> 3 white dice · can use all 3</label></section><section class="modal" id="modal"><div class="modal-card"><h1>Choose Your Player</h1><div class="players" id="players"></div></div></section><button class="return" id="return">← Game Night</button><div class="toast" id="toast"></div>`;
  document.body.insertAdjacentHTML('beforeend', `<style>
    ...
    .cell,.cell.mark{text-shadow:none!important}.cell.mark{height:28px!important;min-width:0!important}.board-foot{min-height:38px}.side-actions{grid-template-columns:1fr}.all-time{padding:8px 9px}.all-time-tabs{display:grid;grid-template-columns:1fr 1fr;gap:4px}.all-time-tabs button,.all-time-filter{padding:6px 2px;border:1px solid #70477a;border-radius:7px;background:#25112d;color:#d7c4a9;font-size:10px;font-weight:900}.all-time-tabs button.active{background:#7e3246;border-color:#c2717e;color:#fff2c2}.all-time-filter{width:100%;margin-top:5px}.all-time-filter.active{background:#4c253f;color:#ffedb0}.all-time-list{margin:7px 1px 0;padding:0;list-style:none;counter-reset:rank}.all-time-list li{counter-increment:rank;display:grid;grid-template-columns:18px 1fr auto;align-items:center;gap:3px;padding:3px 0;border-bottom:1px solid #38213e;color:#ffedc1;font-size:11px;font-weight:800}.all-time-list li::before{content:counter(rank) '.';color:#bf9f6d}.all-time-list li.empty{display:block;color:#bba8b1;font-size:10px;font-weight:600;text-align:center;border:0;padding:6px 0}.all-time-list li.empty::before{display:none}.seat-done{margin-left:3px;border:1px solid #a17b43;border-radius:6px;background:#391943;color:#fff1b9;padding:4px 7px;font-size:10px;font-weight:900}.done{display:block;margin:18px auto 0;border:1px solid #9c7b42;border-radius:8px;background:#311a37;color:#fff4c7;padding:9px 28px;font-weight:900}
  </style>`);
  ...
  function render() {
    ...
    $('#botScoreToggle').textContent=excludeBotScores?'Show Bots':'Hide Bots';
    $('#botScoreToggle').classList.toggle('active',excludeBotScores);
    const scores = state.allTime?.[excludeBotScores ? `${scoreTab}Humans` : scoreTab] || [];
    ...
    $('#seatDone')?.addEventListener('click',event=>{
      // Remove it immediately; if the server rejects the action, act() reloads
      // the table and restores the button when it is still needed.
  ...
  $('#botScoreToggle').onclick=()=>{excludeBotScores=!excludeBotScores;render()};
