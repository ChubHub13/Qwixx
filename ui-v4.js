(() => {
  ...
  document.head.insertAdjacentHTML('beforeend', `<style>
    .row{grid-template-columns:17px repeat(11,minmax(10px,1fr)) 38px!important}.row-score{display:grid;place-items:center;border-radius:3px;background:#151619;color:#fff;font-size:14px;font-weight:900}.cell{font-size:14px!important;font-weight:900!important;opacity:1!important}.row.red .cell.mark{background:var(--red)!important}.row.yellow .cell.mark{background:var(--yellow)!important}.row.green .cell.mark{background:var(--green)!important}.row.blue .cell.mark{background:var(--blue)!important}.cell.mark{color:#fff!important;text-shadow:0 1px 2px #000;font-size:17px!important}
    .done{display:block;margin:13px auto 0;border:1px solid #9c7b42;border-radius:8px;background:#311a37;color:#fff4c7;padding:7px 22px;font-weight:900}
  </style>`);
  ...
  function render() {
    ...
    document.querySelectorAll('.board').forEach((board, index) => {
      ...
      board.querySelectorAll('.row').forEach((row, colorIndex) => {
        ...
        row.insertAdjacentHTML('beforeend', `<span class="row-score">${count * (count + 1) / 2}</span>`);
      });
    });
