(() => {
  ...
  const die = (n,color='white',rerollIndex=null) => `<i class="die ${color}" ${rerollIndex === null ? '' : `data-reroll="${rerollIndex}"`}>${pips(n)}</i>`;
  ...
  function render() {
    ...
    $('#roll').innerHTML=`<h2>${title}</h2>${state.dice?`<div class="dice ${newRoll?'new-roll':''}">${state.dice.white.map((n,index)=>die(n,'white',canReroll?index:null)).join('')+colors.map(c=>die(state.dice[c],c)).join('')}</div>`:''}${nextRoll?'<button class="done" id="nextRoll">Next roll</button>':''}`;
