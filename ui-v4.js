(() => {
  ...
  document.head.insertAdjacentHTML('beforeend', `<style>
    .roll{width:430px!important;min-height:270px!important;padding:22px!important}.roll .die{animation:roll-in .58s cubic-bezier(.2,.8,.2,1) both}.roll .die:nth-child(2){animation-delay:.07s}.roll .die:nth-child(3){animation-delay:.14s}.roll .die:nth-child(4){animation-delay:.21s}.roll .die:nth-child(5){animation-delay:.28s}.roll .die:nth-child(6){animation-delay:.35s}.roll .die:nth-child(7){animation-delay:.42s}@keyframes roll-in{0%{opacity:0;transform:translateY(-48px) rotate(-80deg)}65%{transform:translateY(7px) rotate(13deg)}100%{opacity:1}}
    .row{grid-template-columns:14px repeat(11,minmax(9px,1fr)) 27px 36px!important;gap:2px!important}.row-score,.row-lock{display:grid;place-items:center;border-radius:7px;background:#151619;color:#fff;font-size:14px;font-weight:900}.row-lock{background:#f7f4e7;border:1px solid #999;color:#333;font-size:15px}.cell{font-size:18px!important;font-weight:900!important;opacity:1!important;border-radius:7px!important}.row.red .cell.mark{background:var(--red)!important}.row.yellow .cell.mark{background:var(--yellow)!important}.row.green .cell.mark{background:var(--green)!important}.row.blue .cell.mark{background:var(--blue)!important}.cell.mark{color:#fff!important;text-shadow:0 1px 2px #000;font-size:18px!important}
  ...
  function render() {
    ...
    $('#roll').innerHTML=`<h2>${title}</h2>${state.dice?`<div class="dice">${state.dice.white.map(n=>die(n)).join('')+colors.map(c=>die(state.dice[c],c)).join('')}</div>`:''}${mayFinish?'<button class="done" id="done">Done</button>':''}`;
    ...
    document.querySelectorAll('.board').forEach((board, index) => {
      ...
      board.querySelectorAll('.row').forEach((row, colorIndex) => {
        ...
        row.insertAdjacentHTML('beforeend', `<span class="row-lock">${state.locked[color] ? '🔒' : ''}</span><span class="row-score">${count * (count + 1) / 2}</span>`);
      });
    });
