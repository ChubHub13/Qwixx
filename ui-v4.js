(() => {
  ...
  </style><header class="top"><div class="mark">⚄</div><div class="title">QWIXX<small>Three players · shared-dice table</small></div><i class="spacer"></i><button class="settings" id="settingsButton">Settings</button></header><div class="shell"><aside class="side"><section class="panel round"><div class="eyebrow">ROUND</div><b id="round">Waiting</b><span id="roundHint">Choose a player to begin</span></section><section class="panel"><h2 class="score-title">SCORE · HIGH WINS</h2><div id="people"></div></section><div class="side-actions"><button id="player">Player</button><button id="start">Start<br>Game</button></div></aside><main class="table"><div class="seats" id="seats"></div><div class="boards" id="boards"></div><section class="roll" id="roll"></section></main></div><section class="pop hide" id="settings"><h2>Table settings</h2><label class="option"><input name="count" type="radio" value="2"> 2 white dice</label><label class="option"><input name="count" type="radio" value="3" checked> 3 white dice</label><label class="option"><input name="count" type="radio" value="all3"> 3 white dice · can use all 3</label></section><section class="modal" id="modal"><div class="modal-card"><h1>Choose Your Player</h1><div class="players" id="players"></div></div></section><button class="return" id="return">← Game Night</button><div class="toast" id="toast"></div>`;
  document.head.insertAdjacentHTML('beforeend', `<style>
    .roll{width:430px!important;min-height:270px!important;padding:22px!important}.roll .die{transform:translate(var(--x,0px),var(--y,0px)) rotate(var(--r,0deg))}.roll .die[data-reroll]{cursor:pointer;outline:3px solid #f8dd7a;outline-offset:3px}.roll .dice.new-roll .die{animation:roll-in .72s cubic-bezier(.2,.8,.2,1) var(--delay,0s) both}@keyframes roll-in{0%{opacity:0;transform:translate(0,-70px) rotate(-110deg)}65%{opacity:1;transform:translate(0,7px) rotate(13deg)}100%{opacity:1;transform:translate(var(--x,0px),var(--y,0px)) rotate(var(--r,0deg))}}
    ...
    .cell,.cell.mark{text-shadow:none!important}.cell.mark{height:28px!important;min-width:0!important}.board-foot{min-height:38px}.seat-done{margin-left:3px;border:1px solid #a17b43;border-radius:6px;background:#391943;color:#fff1b9;padding:4px 7px;font-size:10px;font-weight:900}.done{display:block;margin:18px auto 0;border:1px solid #9c7b42;border-radius:8px;background:#311a37;color:#fff4c7;padding:9px 28px;font-weight:900}
  </style>`);
  const $ = s => document.querySelector(s);
  document.head.insertAdjacentHTML('beforeend', `<style>
    .cell.recent{outline:3px solid #000!important;outline-offset:-3px}.row.locked.red .cell:not(.mark){background:#fff!important;color:var(--red)!important}.row.locked.yellow .cell:not(.mark){background:#fff!important;color:var(--yellow)!important}.row.locked.green .cell:not(.mark){background:#fff!important;color:var(--green)!important}.row.locked.blue .cell:not(.mark){background:#fff!important;color:var(--blue)!important}.row.locked.red .row-lock{background:var(--red)!important}.row.locked.yellow .row-lock{background:var(--yellow)!important}.row.locked.green .row-lock{background:var(--green)!important}.row.locked.blue .row-lock{background:var(--blue)!important}.row.locked .row-lock img{filter:invert(1)}
    ...
    @media (min-width:581px) and (max-width:1400px){.shell{min-height:calc(100svh - 61px)}.table{height:calc(100svh - 69px);min-height:0!important;margin:4px;border-radius:24px}.boards{inset:42px 10px 8px;grid-template-rows:165px 1fr 170px;gap:7px}.board{width:83%;padding:4px;border-width:3px}.board.bottom{width:53%}.row{margin:2px 0}.cell,.cell.mark{height:24px!important;font-size:16px!important}.row-score,.row-lock{height:24px;font-size:12px}.row-lock img{width:13px;height:13px}.board-foot{min-height:28px;padding-top:2px}.seat.under-left{top:215px!important}.seat.under-right{top:215px!important}.seat.beside-bottom{bottom:22px!important;right:calc(50% - 34vw)!important}.roll{width:370px!important;min-height:205px!important;padding:14px!important}.roll h2{margin-bottom:8px}.roll .die{width:46px;height:46px;padding:7px}.dice{gap:8px}}
  ...
  function render() {
    ...
    const canReroll = state.phase==='playing' && state.stage==='shared' && state.turn===state.you && !state.rerolled && state.dice?.white?.length===3 && new Set(state.dice.white).size===1;
    if (state.dice && newRoll) {
      const entries = [
        ...state.dice.white.map((value, index) => ({ value, color: 'white', rerollIndex: canReroll ? index : null })),
        ...colors.map(color => ({ value: state.dice[color], color }))
      ];
      diceLayout = entries.sort(() => Math.random() - .5).map((entry, index) => ({
        ...entry, x: Math.round(Math.random() * 18 - 9), y: Math.round(Math.random() * 16 - 8),
        rotate: Math.round(Math.random() * 24 - 12), delay: (index * .07).toFixed(2)
      }));
    }
    $('#roll').innerHTML=`<h2>${title}</h2>${state.dice?`<div class="dice ${newRoll?'new-roll':''}">${diceLayout.map(die).join('')}</div>`:''}${nextRoll?'<button class="done" id="nextRoll">Next roll</button>':''}`;
