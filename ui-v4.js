(() => {
  ...
  document.body.insertAdjacentHTML('beforeend', `<style>
    .roll{width:430px!important;min-height:270px!important;padding:22px!important}.roll .dice{display:grid;justify-items:center;gap:13px}.roll .dice-row{display:flex;justify-content:center;gap:14px;flex-wrap:nowrap}.roll .die{transform:translate(var(--x,0px),var(--y,0px)) rotate(var(--r,0deg))}.roll .die[data-reroll]{cursor:pointer;outline:3px solid #f8dd7a;outline-offset:3px}.roll .dice.new-roll .die{animation:roll-in .72s cubic-bezier(.2,.8,.2,1) var(--delay,0s) both}@keyframes roll-in{0%{opacity:0;transform:translate(0,-70px) rotate(-110deg)}65%{opacity:1;transform:translate(0,7px) rotate(13deg)}100%{opacity:1;transform:translate(var(--x,0px),var(--y,0px)) rotate(var(--r,0deg))}}
  ...
  function render() {
    ...
    if (state.dice && newRoll) {
      const arrangeDice = entries => entries.sort(() => Math.random() - .5).map((entry, index) => ({
      ...
      diceLayout = {
        white: arrangeDice(state.dice.white.map((value, index) => ({ value, color: 'white', rerollIndex: canReroll && (state.cyclingDie === null || state.cyclingDie === index) ? index : null }))),
        colors: arrangeDice(colors.filter(color => !state.locked[color] && state.dice[color] !== undefined).map(color => ({ value: state.dice[color], color })))
      };
    }
