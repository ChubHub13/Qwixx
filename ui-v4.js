  ...
  function render() {
    ...
    document.querySelectorAll('.board').forEach((board, index) => {
      ...
      board.querySelectorAll('.row').forEach((row, colorIndex) => {
        ...
        row.insertAdjacentHTML('beforeend', `<span class="row-lock">${state.locked[color] ? '<img src="/lock-symbol.png" alt="Locked">' : ''}</span><span class="row-score">${count * (count + 1) / 2}</span>`);
      });
    });
