(() => {
  ...
  document.head.insertAdjacentHTML('beforeend', `<style>
    .roll{width:430px!important;min-height:270px!important;padding:22px!important}.roll .dice{gap:14px}.roll .die{transform:translate(var(--x,0px),var(--y,0px)) rotate(var(--r,0deg))}.roll .die[data-reroll]{cursor:pointer;outline:3px solid #f8dd7a;outline-offset:3px}.roll .dice.new-roll .die{animation:roll-in .72s cubic-bezier(.2,.8,.2,1) var(--delay,0s) both}@keyframes roll-in{0%{opacity:0;transform:translate(0,-70px) rotate(-110deg)}65%{opacity:1;transform:translate(0,7px) rotate(13deg)}100%{opacity:1;transform:translate(var(--x,0px),var(--y,0px)) rotate(var(--r,0deg))}}
  ...
  document.head.insertAdjacentHTML('beforeend', `<style>
    ...
    @media (min-width:581px) and (max-width:1400px){.shell{min-height:calc(100svh - 61px)}.table{height:calc(100svh - 69px);min-height:0!important;margin:4px;border-radius:24px}.boards{inset:27px 10px 8px;grid-template-rows:165px 1fr 170px;gap:7px}.board{width:83%;padding:4px;border-width:3px}.board.bottom{width:61%}.row{margin:2px 0}.cell,.cell.mark{height:24px!important;font-size:16px!important}.row-score,.row-lock{height:24px;font-size:12px}.row-lock img{width:13px;height:13px}.board-foot{min-height:28px;padding-top:2px}.seat.under-left{top:200px!important}.seat.under-right{top:200px!important}.seat.beside-bottom{bottom:22px!important;right:calc(50% - 34vw)!important}.roll{width:370px!important;min-height:195px!important;padding:14px!important}.roll h2{margin-bottom:8px}.roll .die{width:46px;height:46px;padding:7px}.roll .dice{gap:12px}}
  ...
  function render() {
    ...
    if (state.dice && newRoll) {
      const entries = [
        ...
        ...colors.filter(color => !state.locked[color] && state.dice[color] !== undefined).map(color => ({ value: state.dice[color], color }))
      ];
      diceLayout = entries.sort(() => Math.random() - .5).map((entry, index) => ({
        ...entry, x: Math.round(Math.random() * 8 - 4), y: Math.round(Math.random() * 10 - 5),
        rotate: Math.round(Math.random() * 16 - 8), delay: (index * .07).toFixed(2)
      }));
    }
