...
const newGame = () => ({
  phase: 'waiting', turn: 0, rollingSeat: 0, stage: 'shared', round: 0, rollId: 0, dice: null, diceLayout: { white: [], colors: [] },
  settings: { communityDice: 3, allThree: true, firstTurnReroll: true }, locked: Object.fromEntries(COLORS.map(c => [c, false])),
  sheets: [blankSheet(), blankSheet(), blankSheet()], highlights: [[], [], []], sharedUsed: [false, false, false], sharedDone: [false, false, false], colorUsed: false, rerolled: false, cyclingDie: null, actions: [[], [], []], lockNotice: null, closingColors: [], turnsTaken: [0, 0, 0], rescueUsed: [false, false, false], rollBaseSheets: null, botTurnResolved: false,
...
function displayName(seat) { return seatNames[seat] || NAMES[seat] || 'Player'; }
...
function snapshot(you) {
  return {
    ...
    seats: NAMES.map((name, seat) => ({ name: displayName(seat), seat, live: isLive(seat), bot: !isLive(seat), score: score(game.sheets[seat]), wins: wins[seat] }))
  };
}
...
function validMark(seat, color, value) {
  ...
  const alreadyChosen = game.closingColors.includes(color);
  const totalRowsClosing = COLORS.filter(row => game.locked[row]).length + game.closingColors.length;
  if (index === 10 && !alreadyChosen && totalRowsClosing >= 2) return false;
...
function addMark(seat, color, value) {
  ...
  if (index === 10) {
    ...
    const closers = game.sheets.map((sheet, player) => sheet.locks[color] ? displayName(player) : null).filter(Boolean);
    ...
    game.prompt = `${displayName(seat)} closed the ${color} row.`;
  }
  return true;
}
function undoMark(seat, color, index) {
  ...
  if (action.index === 10) {
    ...
    if (!game.sheets.some(sheet => sheet.locks[action.color])) game.closingColors = game.closingColors.filter(color => color !== action.color);
  ...
  game.prompt = `${displayName(seat)} took back a mark.`;
  return true;
}
...
function hasAnyClosingChoice(seat) {
  if (hasSharedClosingChoice(seat)) return true;
  return game.dice?.white.some(white => COLORS.some(color => {
    const value = white + game.dice[color];
    return rowValues(color).indexOf(value) === 10 && validMark(seat, color, value);
  }));
}
function cloneSheets(sheets) { return JSON.parse(JSON.stringify(sheets)); }
function hasMarkWithAtMostTwoSkips(seat, color, value) {
  if (!validMark(seat, color, value)) return false;
  const index = rowValues(color).indexOf(value);
  const marks = game.sheets[seat].marks[color];
  const last = marks.length ? Math.max(...marks) : -1;
  return index - last - 1 <= 2;
}
function firstTurnNeedsReroll(seat) {
  if (!game.dice) return false;
  const shared = communityOptions().some(option => COLORS.some(color => hasMarkWithAtMostTwoSkips(seat, color, option.total)));
  const colored = game.dice.white.some(white => COLORS.some(color => hasMarkWithAtMostTwoSkips(seat, color, white + game.dice[color])));
  return !shared && !colored;
}
function rescueEligible(seat) {
  return game.phase === 'playing' && game.stage === 'shared' && isLive(seat) && seat === game.turn && game.settings.firstTurnReroll && game.turnsTaken[seat] === 1 && !game.rescueUsed[seat] && !game.sharedDone[seat] && !game.colorUsed && game.actions[seat].length === 0 && firstTurnNeedsReroll(seat);
}
...
function advanceSharedIfReady(lastDoneSeat) {
  ...
  game.prompt = `${displayName(game.turn)} may roll next.`;
...
function settleShared(lastDoneSeat) {
  ...
  advanceSharedIfReady(lastDoneSeat);
}
function roll(countAsTurn = true) {
  // The display changes only when this player actually rolls, not when the
  // previous turn merely finishes and awaits their roll button.
  game.rollingSeat = game.turn;
  if (countAsTurn) game.turnsTaken[game.turn]++;
  game.round++;
  game.rollId++;
  game.dice = {
  ...
  game.prompt = `${displayName(game.turn)} rolled the community dice.`;
  settleShared();
}
function nextTurn() {
  if (checkForEnd()) return;
...
function scheduleBot() {
  ...
  botTimer = setTimeout(() => {
    ...
    if (game.stage === 'shared') {
      game.stage = 'color';
      game.prompt = `${displayName(game.turn)} is using a colored die.`;
      return scheduleBot();
    }
    botColor(game.turn);
    nextTurn();
...
const server = http.createServer((req, res) => {
  ...
  if (url.pathname === '/api/join' && req.method === 'POST') return readJson(req, body => {
    ...
    const currentSeat = seatForToken(body.token);
    ...
    if (prior && prior.token !== body.token) return fail(res, `${displayName(seat)} is already playing on another device.`);
    const token = prior?.token || crypto.randomBytes(18).toString('hex');
    seats.set(seat, { token, name });
    seatNames[seat] = name;
  ...
  if (url.pathname === '/api/action' && req.method === 'POST') return readJson(req, body => {
    ...
    if (action === 'rename') {
      ...
      if (!name) return fail(res, 'Enter a name for this game.');
      seats.get(seat).name = name;
      seatNames[seat] = name;
      return send(res, { state: snapshot(seat) });
    }
    if (action === 'settings') {
      if (Object.prototype.hasOwnProperty.call(body, 'firstTurnReroll')) {
        ...
        game.prompt = `${displayName(seat)} changed the first-turn reroll setting.`;
        return send(res, { state: snapshot(seat) });
      }
      ...
      game.prompt = `${displayName(seat)} changed the white-dice setting.`;
      return send(res, { state: snapshot(seat) });
    }
    if (action === 'start') {
    ...
    if (action === 'shared') {
      ...
      // The roller retains first choice only for a closing mark. Other players
      // are free to make any ordinary legal mark while that choice is pending.
      ...
      if (seat !== game.turn && requestedIndex === 10 && !game.sharedDone[game.turn] && !game.closingColors.length && !game.colorUsed && hasAnyClosingChoice(game.turn)) {
        return fail(res, `${displayName(game.turn)} chooses a closing color first.`);
      }
    ...
    if (action === 'reroll') {
      ...
      game.prompt = `${displayName(seat)} is choosing a value for one white die.`;
      return send(res, { state: snapshot(seat) });
    }
    if (action === 'firstTurnReroll') {
      if (!rescueEligible(seat)) return fail(res, 'That first-turn reroll is not available.');
      game.rescueUsed[seat] = true;
      game.sheets = cloneSheets(game.rollBaseSheets || game.sheets);
      game.highlights = [[], [], []];
      game.sharedUsed = [false, false, false];
      game.sharedDone = [false, false, false];
      game.actions = [[], [], []];
    ...
    if (action === 'penalty') {
      ...
      game.colorUsed = true;
      game.sharedDone[seat] = true;
      game.prompt = `${displayName(seat)} took a −5 penalty.`;
      settleShared(seat);
      return send(res, { state: snapshot(seat) });
    }
    if (action === 'nextRoll') {
